...
void newImageUpdate(float* fp) 
{
	...
	float p0_z = fancy_dframe.get_distance(646, 317);
	float p1_z = fancy_dframe.get_distance(600, 419);
	float p2_z = fancy_dframe.get_distance(322, 315);

	Vector3 p0 = convert_depth_pixel_to_metric_coordinate(646.0f, 317.0f, p0_z);
	Vector3 p1 = convert_depth_pixel_to_metric_coordinate(600.0f, 419.0f, p1_z);
	Vector3 p2 = convert_depth_pixel_to_metric_coordinate(322.0f, 315.0f, p2_z);
	...
	cv::Mat scale_mat;
	depth_mat.convertTo(scale_mat, CV_8U, -255.0 / 1200.0, 255.0); // 0-10000 -> 255(white)-0(black)
	...
	*/
	...
	// New coordinate
	cv::Mat Mat_out =  tran_M * _Txs2 * input.t();
	Mat_out = Mat_out.t();
	...
	// Calc. new coordinate.
	cv::Mat Mat_out2 = tran_M * _Txs2 * input2.t();
	Mat_out2 = Mat_out2.t();
